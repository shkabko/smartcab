...
class TrafficLight(object):
    ...
    def __init__(self, state=None, period=None):
        ...
        self.period = period if period is not None else random.choice([3, 4, 5]) # why 3,4,5 sec before change
        self.last_updated = 0
    ...
    def __init__(self, learning_rate=None, discount_rate=None, num_dummies=3):
        ...
        for a in self.intersections:
            for b in self.intersections:
                if a == b:
                    continue
                if (abs(a[0] - b[0]) + abs(a[1] - b[1])) == 1:  # L1 distance betweeen 2 points = 1
